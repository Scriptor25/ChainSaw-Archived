...
public class Parser {
    ...
    public Expr parsePrimaryExpr() {
        ...
        return switch (token.type) {
            case IDENTIFIER -> new IdentifierExpr(token.value);
            case NUMBER -> new ConstantExpr(token.value, ConstantExpr.ConstantType.NUMBER);
            case CHAR -> new ConstantExpr(token.value, ConstantExpr.ConstantType.CHAR);
            case STRING -> new ConstantExpr(token.value, ConstantExpr.ConstantType.STRING);
            case PAREN_OPEN -> parseParenExpr();
            case MINUS -> new UnaryExpr("-", parseUnaryExpr());
            case EXCLAM -> new UnaryExpr("!", parseUnaryExpr());
            case TILDE -> new UnaryExpr("~", parseUnaryExpr());
            default -> error(token.line, "undefined token type '%s' ('%s')", token.type, token.value);
        };
    }
